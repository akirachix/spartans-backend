...
class LoanRepayment(models.Model):
    ...
    total_outstanding_loan = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    ...
    def repayment_status(self):
        ...
        if self.payment_date is None and self.amount_remaining <= 0:
            return 'on_time'
        delay_days = (self.payment_date - self.due_date).days if self.payment_date else 0
        if delay_days <= 0:
            return 'on_time'
        elif delay_days <= 7:
            return 'minor_delay'
